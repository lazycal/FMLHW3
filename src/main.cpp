  ...
  const std::vector<double> &D) 
{
  ...
  for (int i = 0; i < m; ++i) eplus += (y[i] != 1) * D[i];
  for (int i = 0; i < m; ++i) eminus += (y[i] != -1) * D[i];
  if (eplus < eminus) {
    min_e = eplus;
    min_h = Base{std::numeric_limits<double>::lowest(), 1, 0};
    for (int i = 0; i < m; ++i) assert(min_h.predict(X[i]) == 1);
  } else {
    min_e = eminus;
    min_h = Base{std::numeric_limits<double>::lowest(), -1, 0};
    for (int i = 0; i < m; ++i) assert(min_h.predict(X[i]) == -1);
  }
  assert(min_e == eval(min_h, X, y, m, n, D));
  for (int j = 0; j < n; ++j)
    for (int s = 1; s >= -1; s -= 2) { // return s if x>=thres
      // cout << ">>>>>>>>>> j="<<j << ", s=" << s << '\n';
      double e = (std::array<double, 2>{eminus, eplus})[(s+1)/2];
      auto idx_j = idx.begin() + j*m;
      for (int i = 0; i < m - 1; ++i) {
        // auto e1=e;
        e += (s*y[idx_j[i]]) * D[idx_j[i]];
        if (X[idx_j[i+1]][j] == X[idx_j[i]][j]) continue; // threshold unchanged
        // auto h=Base{X[idx_j[i+1]][j], s, j};
        // cout << "prev e=" << e1 << ", eval=";
        // cout << eval(h, X, y, m, n, D) << ", e=" << e << ", h=" << h << std::endl;
        // assert(std::abs(eval(h, X, y, m, n, D) - e) < EPS);
        if (min_e > e) {
          min_e = e;
          min_h = Base{X[idx_j[i+1]][j], s, j};
          // assert(std::abs(min_e - eval(min_h, X, y, m, n, D)) < EPS);
        }
      }
    }
  assert(std::abs(min_e - eval(min_h, X, y, m, n, D)) < EPS);
...
{
  std::vector<double> D(m, 1./m), fx(m), fx_test(m_test);
  Ensemble res(n);
  // pre sorting
  ...
  for (int t = 1; t <= T; ++t) {
    ...
    for (int i = 0; i < m_test; ++i) fx_test[i] += h.predict(X_test[i]) * alpha;
    // normalize D
    ...
    if (t % 1 == 0) {
      double test_err = 0;
      for (int i = 0; i < m_test; ++i) test_err += (y_test[i] != sgn(fx_test[i]));
      test_err /= m_test;
      // assert(std::abs(test_err - eval(res, X_test, y_test, m_test, n)) < EPS);
      double train_loss = 0, test_loss = 0;
      auto loss_f = [=](double u) {return use_log ? std::log2(1+std::exp(-u)) : std::exp(-u);};
      for (int i = 0; i < m; ++i) train_loss += loss_f(y[i] * fx[i]) / m;
      for (int i = 0; i < m_test; ++i) test_loss += loss_f(y_test[i] * fx_test[i]) / m_test;
      cout << "t=#" << t << ": error=" << e << ", test error=" << test_err <<
